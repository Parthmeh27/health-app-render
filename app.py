...
def generate_synthetic_data(num_samples):
    ...
    data = {
        ...
        'Sleep Duration': np.random.uniform(4, 10, num_samples),
        'Physical Activity Level': np.random.uniform(1, 10, num_samples),
        'Heart Rate': np.random.uniform(60, 100, num_samples),
        'Daily Steps': np.random.uniform(1000, 15000, num_samples)
    }
...
with tab1:
    ...
    fig, ax = plt.subplots()
    sns.histplot(df['Sleep Duration'], bins=20, kde=True, ax=ax)
    st.pyplot(fig)
    ...
    color_col = "Stress Level" if "Stress Level" in df.columns else None
...
with tab2:
    ...
    Sleep_Duration = st.number_input("Enter Sleep Duration (hours)", min_value=0.0, max_value=15.0, value=7.0)
    ...
    if st.button("Predict Stress Level"):
        diagnosis = predict_stress_level([Age, Sleep_Duration, Physical_Activity_Level, Heart_Rate, Daily_Steps])
